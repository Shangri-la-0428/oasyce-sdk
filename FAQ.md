...
正常路径不再要求手工传播 mnemonic。`oasyce share` 默认把连接文件写到桌面；连接文件路径才是默认多设备入口。这个文件本身会直接声明“这是给另一台 AI / 电脑加入现有 Oasyce 环境的 handoff artifact”，所以主设备侧最自然的动作就是“生成文件并发给另一台电脑上的 AI”，而不是再手工解释安装和 join 步骤。
